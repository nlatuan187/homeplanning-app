...
type EducationContent = {
  ...
  message: string;
  fileName: string;
  type: "CHUNG_CU" | "NHA_DAT"; // Chuẩn hóa type
  location: "HCMC" | "HANOI";     // Chuẩn hóa location
  order: number;
};
...
const EDUCATION_DATA: EducationContent[] = [
  ...
  // --- ĐẤT NỀN (NHÀ ĐẤT) HCM ---
  // Lưu ý: Cần map "NHA_DAT" hoặc "NHA_DAT" thống nhất với Frontend gửi lên
  {
    order: 1,
    type: "NHA_DAT",
    location: "HCMC",
 ...
 *         schema:
 *           type: string
 *         description: Loại dữ liệu (CHUNG_CU, NHA_DAT)
 *       - in: query
 *         name: location
 *         required: true
 *         schema:
...
export async function GET(req: NextRequest) {
  ...
  const filteredData = EDUCATION_DATA.filter(item => {
    // So sánh linh hoạt một chút (ví dụ client gửi 'chung-cu' thì vẫn match 'CHUNG_CU' nếu cần xử lý thêm)
    // Ở đây giả định client gửi đúng 'CHUNG_CU', 'NHA_DAT', 'HN', 'HCM'
    // Map NHA_DAT nếu client gửi NHA_DAT
    const normalizedType = (type === "NHA_DAT" || type === "NHA DAT") ? "NHA_DAT" : type;
