...
export default function ResultsScenarioA({
  ...
  if (showDetails) {
    ...
    // Income components for the target year
    const luongNamMucTieu = Math.round(targetYearProjection.primaryIncome / 12);
    const thuNhapVoChongNamMucTieu = Math.round(targetYearProjection.spouseIncome / 12);
    const thuNhapKhacNamMucTieu = Math.round(targetYearProjection.otherIncome / 12);
    const tongThuThangDisplay = Math.round(targetYearProjection.annualIncome / 12); // Verified total

    // Income components for the current year
    ...
    const chiPhiSinhHoatNamMucTieu = Math.round(targetYearProjection.annualExpenses / 12);

    const soTienVay = Math.round(targetYearProjection.loanAmountNeeded);
    const tienTraGopHangThang = Math.round(targetYearProjection.monthlyPayment);
    const tongChiThang = chiPhiSinhHoatNamMucTieu + tienTraGopHangThang;
    const ketLuanBuffer = Math.round(targetYearProjection.buffer);
      ...
      <div className="w-full space-y-6 p-4 bg-black text-slate-100"> {/* Removed max-w classes */}
        ...
        <div className="bg-red-500/20 p-4 rounded-lg text-sm md:text-base border border-red-500/50">
          ...
          <p className="text-slate-200">
            Tổng thu là {tongThuThangDisplay.toLocaleString()} triệu/tháng, {tongThuThangDisplay <= tongChiThang ? "nhỏ hơn hoặc bằng" : "lớn hơn"} tổng chi là {tongChiThang.toLocaleString()} triệu/tháng, đồng nghĩa hàng tháng bạn {ketLuanBuffer < 0 ? `thâm hụt ${Math.abs(ketLuanBuffer).toLocaleString()} triệu.` : `chỉ còn dư ${ketLuanBuffer.toLocaleString()} triệu, không đủ để tạo một khoản đệm an toàn.`}
          </p>
        </div>
