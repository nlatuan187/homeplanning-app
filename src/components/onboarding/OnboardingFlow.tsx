...
export default function OnboardingFlow({ planId }: OnboardingFlowProps) {
  ...
  const renderSection = () => {
    switch (currentSection) {
      ...
      case 'assumptions':
        return <Assumption plan={planState as Plan} onFinalChoice={() => {}} onConfirm={() => {}} step="intro" setStep={() => {}} assumptionStep={0} onNext={() => {}} onPrev={() => {}} result={null} assumptions={{pctSalaryGrowth: 0, pctHouseGrowth: 0, loanInterestRate: 0}} onSliderChange={() => {}} chartData={[]}/>;
      default:
        return <QuickCheck onCompleted={handleQuickCheckCompleted} />;
    }
  };
