...
export default function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Bước 1: Gọi API backend để tạo người dùng trên Clerk
      ...
      // Bước 2: Nếu tạo thành công, tiến hành đăng nhập người dùng
      ...
      if (result.status === "complete") {
        ...
        // Xử lý trường hợp có thể cần bước xác thực bổ sung
        toast.error("Đã xảy ra lỗi khi đăng nhập sau khi đăng ký.");
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Button type="submit" disabled={isSubmitting} className="cursor-pointer w-full bg-cyan-500 hover:bg-cyan-600 font-bold text-base text-white">
      ...
      <div className="space-y-3">
        <Button onClick={() => signInWith('oauth_google')} variant="outline" className="cursor-pointer w-full flex items-center justify-center gap-2 bg-slate-900 border-slate-700 hover:bg-slate-800 hover:text-white">
          <GoogleIcon />
          Tiếp tục với Google
        </Button>
        <Button onClick={() => signInWith('oauth_apple')} variant="outline" className="cursor-pointer w-full flex items-center justify-center gap-2 bg-slate-900 border-slate-700 hover:bg-slate-800 hover:text-white">
          <AppleIcon />
          Tiếp tục với Apple
        </Button>
      </div>
